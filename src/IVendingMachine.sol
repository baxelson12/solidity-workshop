...
interface IVendingMachine {
    ...
    event ItemAdded(bytes3 indexed location, uint128 price, uint64 stock);
    /// Emitted when an item is removed
    event ItemRemoved(bytes3 indexed location);
    /// Emitted when an item is restocked
    event ItemRestocked(bytes3 indexed location, uint64 stock);
    /// Emitted when an item is repriced
    event ItemRepriced(bytes3 indexed location, uint128 price);
    /// Emitted when an item is purchased
    event ItemPurchased(bytes3 indexed location, uint64 stock, uint64 sold);
    ...
    function addInventory(bytes3 location, uint128 price, uint64 stock) external;

    /// @notice Remove the specified Item in the vending machine
    /// @param location The location of the item within the vending machine
    /// @dev Location is defined by {Alpha}{Numeric} e.g D3
    function removeInventory(bytes3 location) external;
    ...
    /// @dev Location is defined by {Alpha}{Numeric} e.g D3
    function restock(bytes3 location, uint64 stock) external;

    /// @notice Update the price of the specified item
    /// @param location The location of the item within the vending machine
    /// @param price The new price of the item in USD with 8 decimals (10^18)
    /// @dev Location is defined by {Alpha}{Numeric} e.g D3
    function reprice(bytes3 location, uint128 price) external;

    /// @notice Purchase the specified Item
    /// @param location The location of the item within the vending machine
    /// @dev Location is defined by {Alpha}{Numeric} e.g D3
    function purchase(bytes3 location) external payable;
